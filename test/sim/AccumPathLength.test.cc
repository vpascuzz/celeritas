...
TEST_F(AccumPathLengthTest, all)
{
    ...
    // TODO: enable when REQUIRE throws instead of `assert`ing
#if 0
    // Require apl to be in a valid state before operating/comparing
    EXPECT_THROW({ bool failure = (apl == apl); }, nemesis::assertion);
    EXPECT_THROW(apl += 3.0, nemesis::assertion);
    EXPECT_THROW(apl - apl, nemesis::assertion);
#endif
